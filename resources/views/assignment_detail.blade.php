            ...
            <div class="bg-white/90 backdrop-blur-2xl border-b border-slate-200/60 sticky top-0 z-40 px-3 sm:px-4 md:px-8 py-3 sm:py-4 shadow-sm transition-all w-full">
                <div class="max-w-7xl mx-auto flex items-center justify-between relative h-10 sm:h-12 md:h-14">
                    ...
                    <div class="flex items-center gap-2 sm:gap-4 relative z-10 w-auto justify-start">
                        <a href="{{ route('course.assignments', $kelas->id) }}" class="w-9 h-9 sm:w-11 sm:h-11 md:w-12 md:h-12 rounded-full bg-slate-100 hover:bg-blue-600 text-slate-500 hover:text-white flex items-center justify-center transition-all duration-300 shadow-sm group border border-slate-200 hover:border-blue-600 relative cursor-pointer active:scale-95 shrink-0">
                    ...
                    <div class="flex items-center justify-end gap-1.5 sm:gap-3 pl-2 sm:pl-4 relative z-10 shrink-0">
                        ...
                        <span id="status-desc" class="hidden md:block text-[8px] sm:text-[9px] font-black text-slate-400 uppercase tracking-widest text-left w-12 sm:w-20">Siap</span>
                    </div>
                </div>
            </div>
            ...
            <div class="max-w-6xl mx-auto w-full px-3 sm:px-4 md:px-8 py-4 sm:py-6 md:py-8 space-y-4 sm:space-y-6 md:space-y-8 pb-20">
                ...
                <div class="bg-white rounded-[1.5rem] sm:rounded-[2rem] p-5 sm:p-6 lg:p-8 border border-slate-200 shadow-sm relative overflow-hidden" data-aos="fade-up" data-aos-duration="600">
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-4 sm:mb-6">
                        ...
                        <button onclick="navigasiKe(5)" class="w-fit flex items-center gap-2 bg-blue-50 text-blue-600 px-2.5 sm:px-3 py-1.5 rounded-lg text-[8px] sm:text-[9px] font-bold uppercase tracking-widest hover:bg-blue-100 transition-all cursor-pointer active:scale-95 group border border-blue-100">
                    ...
                    <div id="submission-form" class="{{ $isSubmittedStatus ? 'hidden' : 'block' }}">
                        ...
                        <form action="{{ route('mahasiswa.assignment.store', ['kelas' => $kelas->id, 'assignment' => $assignment->id]) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 mb-4 sm:mb-6">
                                ...
                                <div class="flex flex-col h-full">
                                    <label class="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 sm:mb-2 flex items-center justify-between">
                                        <span class="flex items-center gap-1.5 sm:gap-2"><span class="bg-slate-200 text-slate-600 w-4 h-4 rounded flex items-center justify-center">7</span> Ketik Jawaban</span>
                                        <span id="typing-indicator" class="hidden text-red-500 animate-pulse text-[8px] sm:text-[9px]">Mendengarkan...</span>
                                    </label>
                                    <textarea name="text_submission" id="text-submission" class="flex-1 w-full bg-slate-50 border border-slate-300 rounded-xl sm:rounded-2xl p-3 sm:p-4 text-xs sm:text-sm font-medium text-slate-700 placeholder-slate-400 focus:outline-none focus:border-blue-400 focus:bg-white transition-all resize-none min-h-[6rem] sm:min-h-[8rem]" placeholder="Sebut 7 mendikte..."></textarea>
                                </div>

                                <div class="flex flex-col h-full">
                                    <label class="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 sm:mb-2 flex items-center justify-between">
                                        <span class="flex items-center gap-1.5 sm:gap-2"><span class="bg-slate-200 text-slate-600 w-4 h-4 rounded flex items-center justify-center">8</span> Rekam Suara</span>
                                    ...
                                    <div class="flex-1 w-full bg-slate-50 border border-slate-300 rounded-xl sm:rounded-2xl p-3 sm:p-4 flex flex-col items-center justify-center gap-1.5 sm:gap-2 transition-all min-h-[6rem] sm:min-h-[8rem]" id="assignmentVoiceBox">
                                        ...
                                        <span id="assignmentVoiceLabel" class="text-[10px] sm:text-xs font-bold text-slate-500 text-center">Sebut 8 merekam</span>
                            ...
                            <button type="submit" id="btnSubmitForm" class="w-full bg-blue-600 text-white rounded-lg sm:rounded-xl py-3 sm:py-4 font-black text-[10px] sm:text-xs uppercase tracking-[0.1em] sm:tracking-[0.2em] shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all hover:scale-[1.01] active:scale-95 flex items-center justify-center gap-2 sm:gap-3 relative overflow-hidden">
                                <span class="bg-white/20 text-white w-5 h-5 sm:w-6 sm:h-6 rounded flex items-center justify-center font-black text-[9px] sm:text-[10px]">9</span>
                ...
                <div data-aos="fade-up" data-aos-duration="600" data-aos-delay="200" class="bg-white rounded-[1.5rem] sm:rounded-[2rem] md:rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-[500px] sm:h-[600px] mt-6 sm:mt-8">
                    <div class="p-4 sm:p-5 md:p-6 md:px-8 border-b border-slate-100 flex flex-col sm:flex-row justify-between sm:items-center gap-2 sm:gap-0 bg-slate-50/50 z-10">
                        ...
                        <button onclick="navigasiKe(14)" class="w-fit text-[8px] sm:text-[9px] font-bold bg-blue-100 text-blue-700 px-2.5 sm:px-3 py-1.5 rounded-full flex items-center gap-1.5 cursor-pointer hover:bg-blue-200 transition-colors">
                            Sebut "14" Baca Chat
                        </button>
                    </div>
                    ...
                    <div id="chatContainer" class="flex-1 p-3 sm:p-4 md:p-6 flex flex-col gap-4 sm:gap-6 overflow-y-auto custom-scrollbar bg-slate-50/30">
                        ...
                        @empty
                            <div id="emptyChat" class="h-full flex flex-col items-center justify-center text-center opacity-70">
                                ...
                                <p class="text-[8px] sm:text-[10px] text-slate-400 uppercase tracking-widest mt-1">Sebut 10 untuk mendikte pesan ke dosen.</p>
                            </div>
                        @endforelse
                    </div>
                        ...
                        <form id="chatForm" action="{{ route('mahasiswa.assignment.message.store', [ 'kelas' => $kelas->id, 'assignment' => $assignment->id ]) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="relative flex items-center gap-1.5 sm:gap-2 md:gap-3 bg-slate-50 p-1.5 sm:p-2 md:p-3 rounded-2xl sm:rounded-[1.25rem] border border-slate-200 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all shadow-sm w-full">
                                ...
                                <div id="uploadImageContainer" class="relative shrink-0 flex items-center">
                                    ...
                                    <span class="absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 bg-slate-900 text-white text-[7px] sm:text-[8px] font-black px-1.5 py-0.5 rounded-md shadow-sm border border-white">11</span>
                                </div>
                                ...
                                <div id="normalInputWrapper" class="flex-1 min-w-0 relative flex items-center bg-white sm:bg-transparent rounded-xl sm:rounded-none px-2 sm:px-0 shadow-sm sm:shadow-none py-0.5 sm:py-0">
                                    <div class="absolute left-2 text-[8px] sm:text-[10px] font-black text-white bg-slate-900 px-1.5 py-0.5 rounded-md shadow-sm hidden sm:block z-10">10</div>
                                    <input type="text" name="message" id="messageInput" placeholder="Sebut 10 ketik..." autocomplete="off" class="w-full bg-transparent text-[10px] sm:text-xs md:text-sm font-medium text-slate-700 placeholder-slate-400 focus:outline-none transition-all py-1 sm:py-1.5 pl-1 sm:pl-8" />
                                ...
                                <div id="recordBtnContainer" class="relative shrink-0 flex items-center">
                                    ...
                                    <span class="absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 bg-slate-900 text-white text-[7px] sm:text-[8px] font-black px-1.5 py-0.5 rounded-md shadow-sm border border-white">12</span>
                                </div>
                                ...
                                <div class="relative shrink-0 flex items-center ml-0.5 sm:ml-1 md:ml-0">
                                    ...
                                    <span class="absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 bg-slate-900 text-white text-[7px] sm:text-[8px] font-black px-1.5 py-0.5 rounded-md shadow-sm border border-white z-10">13</span>
                                </div>
                            </div>
                        </form>
                    </div>
            ...
            function previewImage(input) {
                if (input.files && input.files[0]) {
                    ...
                    reader.onload = function(e) {
                        ...
                        bicara("Gambar dipilih. Sebut tiga belas untuk mengirim pesan chat.", () => { rec.start(); });
                    }
                ...
                recordBtn.addEventListener('click', async () => {
                    if (!chatMediaRecorder || chatMediaRecorder.state === "inactive") {
                        ...
                        chatMediaRecorder.onstop = () => {
                            ...
                            setTimeout(() => {
                                bicara("Suara chat disimpan. Sebutkan kirim atau batal.", () => { mulaiMendengar(); });
                            }, 500);
                        };
                    ...
                    try {
                        ...
                        if(res.ok && data.success) {
                            ...
                            if(d) {
                                ...
                                if(d.image) media += `<img src="/storage/${d.image}" onerror="this.style.display='none'" class="mt-1.5 sm:mt-2 rounded-lg sm:rounded-xl max-w-full md:max-w-xs border border-white/20">`;
                                if(d.voice) media += `<div class="mt-1.5 sm:mt-2 flex items-center gap-2 bg-white/20 border-white/30 p-1.5 rounded-xl border w-[160px] sm:w-[180px] md:w-[200px]"><button type="button" onclick="togglePlay('${uid}')" id="btn-${uid}" class="w-5 h-5 sm:w-6 h-6 shrink-0 flex items-center justify-center rounded-full bg-white text-blue-600 shadow text-[8px] sm:text-[9px]">▶</button><div id="${uid}" class="flex-1 h-3 sm:h-4" data-audio="/storage/${d.voice}"></div></div>`;
                                
                                const html = `<div class="flex justify-end chat-bubble-new chat-bubble"><div class="flex gap-2 md:gap-3 items-end max-w-[90%] sm:max-w-[85%] md:max-w-[70%] flex-row-reverse"><img src="${myAvatarAJAX}" onerror="this.src='${fallbackAvatar}'" class="w-6 h-6 sm:w-8 h-8 rounded-full shrink-0 shadow-sm border border-slate-100 object-cover"><div class="flex flex-col items-end min-w-0"><p class="text-[8px] sm:text-[9px] md:text-[10px] font-bold mb-1 px-1 text-slate-400 sender-name">Anda</p><div class="p-2.5 sm:p-3 md:p-4 rounded-xl sm:rounded-2xl shadow-sm text-[11px] sm:text-xs md:text-[13px] bg-blue-600 text-white rounded-tr-none border border-blue-700 w-fit max-w-full">${d.message ? `<p class="whitespace-pre-wrap break-words leading-relaxed message-text">${d.message}</p>` : ''}${media}</div><p class="text-[7px] sm:text-[8px] md:text-[9px] mt-1 sm:mt-1.5 px-1 font-bold text-slate-400">${d.time}</p></div></div></div>`;
