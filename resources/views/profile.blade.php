    ...
    <body class="m-0 font-['Plus_Jakarta_Sans'] bg-[#f8fafc] h-[100dvh] flex flex-col lg:flex-row border-box text-slate-800 overflow-hidden">
            ...
            <header class="bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-4 md:px-8 py-3 sm:py-6 sticky top-0 z-30 shrink-0">
                <div class="max-w-7xl mx-auto flex items-center justify-between h-10 sm:h-14">
                    <div class="flex items-center gap-2 sm:gap-4">
                        ...
                        <div>
                            <h2 class="text-lg sm:text-2xl font-extrabold text-slate-900 tracking-tight leading-none">Identitas Diri</h2>
                            <p class="text-[9px] sm:text-sm font-medium text-slate-500 mt-1">Informasi akademik dan personal.</p>
                        </div>
                    </div>
            ...
            <div class="p-4 sm:p-6 lg:p-10 max-w-6xl mx-auto w-full space-y-6 sm:space-y-8">
                ...
                @if($errors->any())
                    <div class="p-3 sm:p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl sm:rounded-2xl font-bold text-xs sm:text-sm safe-fade-in flex flex-col gap-2 shadow-sm">
                        <div class="flex items-center gap-2 sm:gap-3">
                            <div class="p-1 sm:p-1.5 bg-red-100 rounded-md sm:rounded-lg">
                                <svg class="w-4 h-4 sm:w-5 sm:h-5 text-red-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </div>
                            Ada kesalahan pada inputan Anda:
                        </div>
                        <ul class="list-disc list-inside pl-8 sm:pl-10 text-[10px] sm:text-xs font-medium">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-8">
                    ...
                    <div data-aos="fade-up" class="bg-white p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col items-center text-center relative overflow-hidden">
                        <div class="w-32 h-32 sm:w-40 sm:h-40 rounded-[1.5rem] sm:rounded-[2rem] border-4 border-blue-50 p-1 mb-4 sm:mb-6 relative group">
                            ...
                            @endphp
                            <img src="{{ $foto }}" onerror="this.onerror=null; this.src='{{ $avatarUrl }}';" class="w-full h-full object-cover rounded-xl sm:rounded-[1.8rem]" />
                        </div>
                        
                        <h2 class="text-lg sm:text-xl font-black text-slate-900 tracking-tight">{{ $mahasiswa->nama }}</h2>
                        <p class="text-xs sm:text-sm font-bold text-slate-400 mt-1">{{ $mahasiswa->nim }}</p>
                        
                        <div class="mt-4 sm:mt-6 flex flex-wrap justify-center gap-2">
                            <span class="px-3 py-1.5 sm:px-4 sm:py-2 bg-emerald-50 text-emerald-600 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest border border-emerald-100">
                                {{ $mahasiswa->status ?? 'Aktif' }}
                            </span>
                            <span class="px-3 py-1.5 sm:px-4 sm:py-2 bg-blue-50 text-blue-600 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase tracking-widest border border-blue-100">
                        ...
                        <div class="relative z-10 space-y-6 sm:space-y-8">
                            ...
                            <div>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                                    <div class="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100">
                                        <label class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Program Studi</label>
                                        <p class="text-sm sm:text-base font-bold text-slate-800">{{ $mahasiswa->prodi ?? '-' }}</p>
                                    </div>
                                    <div class="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100">
                                        <label class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Fakultas</label>
                                        <p class="text-sm sm:text-base font-bold text-slate-800">{{ $mahasiswa->fakultas ?? '-' }}</p>
                                    </div>
                                    <div class="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100">
                                        <label class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Tahun Masuk</label>
                                        <p class="text-sm sm:text-base font-bold text-slate-800">{{ $mahasiswa->tahun_masuk ?? '-' }}</p>
                                    </div>
                                    <div class="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100">
                                        <label class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Email Kampus</label>
                                        <p class="font-bold text-blue-600 text-xs sm:text-sm truncate">{{ $mahasiswa->email_kampus ?? '-' }}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-6">
                                    ...
                                    <button onclick="bukaModalEdit()" class="w-fit text-[9px] sm:text-[10px] bg-slate-100 hover:bg-blue-100 text-blue-600 font-bold px-2.5 py-1.5 sm:px-3 sm:py-1.5 rounded-lg uppercase tracking-widest transition-colors flex items-center">
                                        <span class="bg-blue-600 text-white px-1.5 py-0.5 rounded mr-1.5">1</span> Edit Kontak
                                    </button>
                                </div>
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                                    <div class="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100">
                                        <label class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Nomor Handphone</label>
                                        <p class="text-sm sm:text-base font-bold text-slate-800">{{ $mahasiswa->no_hp ?? '-' }}</p>
                                    </div>
                                    <div class="bg-slate-50 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100">
                                        <label class="text-[8px] sm:text-[9px] font-bold text-slate-400 uppercase tracking-widest block mb-1">Email Pribadi</label>
                                        <p class="font-bold text-slate-800 text-xs sm:text-sm truncate">{{ $mahasiswa->email_pribadi ?? '-' }}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div data-aos="fade-up" data-aos-delay="200" class="bg-gradient-to-r from-slate-900 to-slate-800 rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-xl text-white flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div class="flex items-center gap-4 sm:gap-6">
                        ...
                        <div>
                            <h3 class="text-lg sm:text-xl font-black tracking-tight">Keamanan Akun</h3>
                            <p class="text-slate-300 text-xs sm:text-sm font-medium mt-0.5">Ubah kata sandi secara berkala untuk perlindungan ekstra.</p>
                        </div>
                    </div>
                    <button onclick="bukaModalPassword()" class="w-full md:w-auto px-6 py-3 sm:px-8 sm:py-4 bg-white text-slate-900 rounded-xl font-black text-[9px] sm:text-[10px] uppercase tracking-widest hover:bg-blue-50 transition-all cursor-pointer flex items-center justify-center gap-2">
        ...
        <div id="modalEdit" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
            ...
            <div class="bg-white rounded-[1.5rem] sm:rounded-[2rem] w-full max-w-md p-6 sm:p-8 relative z-10 shadow-2xl safe-fade-in max-h-[90vh] overflow-y-auto custom-scrollbar">
                ...
                <form action="{{ url('/mahasiswa/profile/update') }}" method="POST" class="space-y-4 sm:space-y-5">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-[10px] sm:text-xs font-bold text-slate-500 mb-1 sm:mb-2">Nomor Handphone</label>
                        <input type="text" name="no_hp" value="{{ $mahasiswa->no_hp }}" class="w-full bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm font-bold text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all" placeholder="08xxxxxxxx">
                    </div>
                    <div>
                        <label class="block text-[10px] sm:text-xs font-bold text-slate-500 mb-1 sm:mb-2">Email Pribadi</label>
                        <input type="email" name="email_pribadi" value="{{ $mahasiswa->email_pribadi }}" class="w-full bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm font-bold text-slate-800 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all" placeholder="email@gmail.com">
                    </div>
                    <button type="submit" class="w-full bg-blue-600 text-white font-black py-3 sm:py-4 rounded-lg sm:rounded-xl text-[10px] sm:text-xs uppercase tracking-widest hover:bg-blue-700 transition-all mt-2 sm:mt-4">
                        Simpan Perubahan
                    </button>
                </form>
            </div>
        ...
        <div id="modalPassword" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
            ...
            <div class="bg-white rounded-[1.5rem] sm:rounded-[2rem] w-full max-w-md p-6 sm:p-8 relative z-10 shadow-2xl safe-fade-in max-h-[90vh] overflow-y-auto custom-scrollbar">
                ...
                <h3 class="text-xl sm:text-2xl font-black text-slate-900 mb-1 sm:mb-2">Ganti Password</h3>
                <p class="text-xs sm:text-sm text-slate-500 mb-4 sm:mb-6">Silakan masukkan password lama dan password baru Anda.</p>
                
                <form action="{{ url('/mahasiswa/profile/password') }}" method="POST" class="space-y-3 sm:space-y-4">
                    @csrf
                    @method('PUT')
                    <div>
                        <label class="block text-[10px] sm:text-xs font-bold text-slate-500 mb-1 sm:mb-2">Password Lama</label>
                        <input type="password" name="old_password" required class="w-full bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm font-bold text-slate-800 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition-all">
                    </div>
                    <div>
                        <label class="block text-[10px] sm:text-xs font-bold text-slate-500 mb-1 sm:mb-2">Password Baru</label>
                        <input type="password" name="new_password" required minlength="8" class="w-full bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm font-bold text-slate-800 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition-all">
                    </div>
                    <div>
                        <label class="block text-[10px] sm:text-xs font-bold text-slate-500 mb-1 sm:mb-2">Konfirmasi Password Baru</label>
                        <input type="password" name="new_password_confirmation" required minlength="8" class="w-full bg-slate-50 border border-slate-200 rounded-lg sm:rounded-xl px-3 py-2 sm:px-4 sm:py-3 text-sm font-bold text-slate-800 focus:outline-none focus:border-orange-500 focus:ring-2 focus:ring-orange-100 transition-all">
                    </div>
                    <button type="submit" class="w-full bg-slate-900 text-white font-black py-3 sm:py-4 rounded-lg sm:rounded-xl text-[10px] sm:text-xs uppercase tracking-widest hover:bg-orange-600 transition-all mt-2 sm:mt-4">
                        Update Password
                    </button>
                </form>
            </div>
