            ...
            <header class="bg-white/80 backdrop-blur-xl border-b border-slate-200/60 px-4 md:px-8 py-3 sm:py-6 sticky top-0 z-20 shrink-0">
                <div class="max-w-7xl mx-auto flex items-center justify-between h-10 sm:h-14">
                    ...
                    <div class="flex items-center gap-2 sm:gap-4">
                        <a href="{{ route('dosen.messages') }}" class="relative p-1.5 sm:p-2 text-slate-400 hover:text-blue-600 transition-all cursor-pointer">
                            <svg class="w-5 h-5 sm:w-6 sm:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path>
                            </svg>
                            @if(isset($unreadMessageCount) && $unreadMessageCount > 0)
                            <span class="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 w-2 h-2 bg-red-500 rounded-full animate-pulse border-2 border-white z-10"></span>
                            @endif
                        </a>
                    </div>
                </div>
            ...
            <div class="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto w-full space-y-4 sm:space-y-6 pb-20">
                ...
                <div class="space-y-3 sm:space-y-4">
                    @forelse ($notifications as $notif)
                        ...
                        {{-- LINK DIUBAH AGAR LANGSUNG MENUJU CONTROLLER read() YANG AKAN MENGURUS MARK-AS-READ DAN REDIRECT --}}
