<style>
    ...
    #a11y-trigger {
        ...
        cursor: grab; /* Kursor grab untuk indikasi bisa digeser */
        display: flex; 
        align-items: center; 
        justify-content: center;
        padding: 0;
        /* Hapus transition transform agar drag mulus, tambahkan hover effect di svg */
    }
    ...
    /* Ikon Aksesibilitas Internasional (Mirip gambar referensi) */
    #a11y-trigger svg { 
        width: 38px; 
        height: 38px; 
        fill: white; 
    ...
    /* PANEL UTAMA (Posisi tetap di kanan, tidak ikut drag) */
    #a11y-panel {
        position: fixed; 
        top: 0; 
        right: -450px; 
    ...
    <div class="a11y-content">
        ...
        <div class="a11y-grid-3">
            <button class="a11y-card" data-action="efek-suara" aria-pressed="false">
                ...
                <div class="a11y-card-title">Mode Suara</div>
            </button>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let hasDragged = false; // Penanda apakah tombol benar-benar digeser atau cuma diklik

    // Muat posisi tombol yang tersimpan
    const savedLeft = localStorage.getItem('a11y_btn_left');
    const savedTop = localStorage.getItem('a11y_btn_top');
    if (savedLeft && savedTop) {
        trigger.style.left = savedLeft;
        trigger.style.top = savedTop;
    ...
    trigger.addEventListener('mousedown', (e) => {
        ...
        let shiftX = e.clientX - trigger.getBoundingClientRect().left;
        let shiftY = e.clientY - trigger.getBoundingClientRect().top;

        function moveAt(pageX, pageY) {
            trigger.style.left = pageX - shiftX + 'px';
            trigger.style.top = pageY - shiftY + 'px';
            trigger.style.bottom = 'auto';
            trigger.style.right = 'auto';
        }
        ...
        function onMouseMove(event) {
            isDragging = true;
            hasDragged = true; // Tandai bahwa pengguna sedang menggeser
        ...
        document.addEventListener('mouseup', () => {
            if(isDragging) {
                ...
                // Simpan posisi baru jika digeser
                if(hasDragged) {
                    localStorage.setItem('a11y_btn_left', trigger.style.left);
                    localStorage.setItem('a11y_btn_top', trigger.style.top);
                }
            }
        }, { once: true });
    });
    ...
    trigger.ondragstart = function() { return false; };
    ...
    trigger.addEventListener('click', (e) => {
        // Hanya buka panel jika pengguna tidak sedang menggeser tombol
        if (!hasDragged) {
            togglePanel();
        }
    });
    ...
    resetBtn.addEventListener('click', () => {
        ...
        // Reset posisi tombol ke pojok kanan bawah
        trigger.style.left = '';
        trigger.style.top = '';
        trigger.style.bottom = '30px';
        trigger.style.right = '30px';
        localStorage.removeItem('a11y_btn_left');
        localStorage.removeItem('a11y_btn_top');
        
        umumkan("Semua pengaturan aksesibilitas dan posisi tombol telah diatur ulang.");
    });
});
</script>