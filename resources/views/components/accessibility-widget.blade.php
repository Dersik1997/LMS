<style>
    ...
    /* Perkecil Teks (4 Level) */
    body.efek-teks-kecil-1 *:not(#a11y-panel *) { font-size: 0.9em !important; }
    body.efek-teks-kecil-2 *:not(#a11y-panel *) { font-size: 0.8em !important; }
    body.efek-teks-kecil-3 *:not(#a11y-panel *) { font-size: 0.7em !important; }
    body.efek-teks-kecil-4 *:not(#a11y-panel *) { font-size: 0.6em !important; }
    ...
    body.efek-kontras-3 *:not(#a11y-panel *) { background-color: #000 !important; color: #ffff00 !important; }
    body.efek-kontras-3 *:not(#a11y-panel *):not(a) { border-color: #ffff00 !important; }

    /* Rata Tulisan */
    body.efek-rata-kiri *:not(#a11y-panel *) { text-align: left !important; display: block !important; }

    /* Sembunyi Gambar */
    body.efek-sembunyi-gambar img:not(#a11y-panel *), 
    body.efek-sembunyi-gambar video, 
    body.efek-sembunyi-gambar [style*="background-image"] { opacity: 0 !important; visibility: hidden !important; }

    /* Tinggi Garis (3 Level) */
    ...
    /* Henti Animasi (Membekukan transisi & animasi) */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function terapkan(aksi, step) {
        ...
        // Hapus semua class level yang mungkin ada
    ...
    cards.forEach(card => {
        ...
        card.addEventListener('click', () => {
            if(aksi.startsWith('profil-')) {
                ...
                // Rotasi step (0 -> 1 -> 2 -> 3 -> 0)
                s = m > 0 ? (s >= m ? 0 : s + 1) : (s === 0 ? 1 : 0);
                
                card.dataset.currentStep = s;
                card.className = 'a11y-card'; // reset class aktif/step
                if(s > 0) {
                    card.classList.add('aktif');
    ...
    resetBtn.addEventListener('click', () => { 
        cards.forEach(c => {
            terapkan(c.dataset.action, 0);
            c.className = 'a11y-card';
            c.dataset.currentStep = 0;
        });
        localStorage.clear();
    });
