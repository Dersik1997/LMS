...
<html lang="id" class="h-full">
    ...
    <body class="m-0 font-['Plus_Jakarta_Sans'] bg-[#f8fafc] min-h-full flex flex-col border-box overflow-x-hidden text-slate-800">
        ...
        <div class="fixed top-0 left-0 w-full h-full overflow-hidden -z-10 pointer-events-none">
            <div class="absolute top-[-10%] right-[-5%] w-64 md:w-[400px] h-64 md:h-[400px] bg-emerald-100/40 rounded-full blur-3xl opacity-50"></div>
            <div class="absolute bottom-[-10%] left-[-10%] w-64 md:w-[400px] h-64 md:h-[400px] bg-blue-50/40 rounded-full blur-3xl opacity-50"></div>
        </div>
        ...
        <main class="flex-1 flex flex-col h-screen overflow-y-auto custom-scrollbar relative w-full">
            ...
            <div class="bg-white/90 backdrop-blur-2xl border-b border-slate-200/60 sticky top-0 z-40 px-4 md:px-8 py-3 sm:py-4 shadow-sm transition-all w-full cursor-pointer" id="voice-header" title="Ketuk 2x untuk memotong suara sistem">
                <div class="max-w-7xl mx-auto flex items-center justify-between relative h-10 sm:h-12 md:h-14">
                    ...
                    <div class="text-center absolute left-1/2 transform -translate-x-1/2 w-[50%] sm:w-[60%] md:max-w-md mt-0 pointer-events-none">
                        <h1 class="text-sm sm:text-lg md:text-2xl font-extrabold text-slate-900 tracking-tight leading-tight truncate">Gabung Kelas Baru</h1>
                        <div class="flex items-center justify-center gap-1 sm:gap-2 mt-0.5 sm:mt-1">
                            <span class="text-[7px] sm:text-[9px] md:text-[10px] font-bold text-blue-600 uppercase tracking-widest bg-blue-100 px-1.5 sm:px-2 py-0.5 rounded-md">Mahasiswa</span>
                        </div>
                    </div>
            ...
            <div class="max-w-4xl mx-auto w-full p-4 sm:p-6 flex flex-col items-center justify-center min-h-[80vh] pb-20">
                <div class="bg-white rounded-[2rem] sm:rounded-[3rem] shadow-xl shadow-slate-100 border border-slate-200 p-6 sm:p-8 md:p-12 w-full relative overflow-hidden">
                    <div class="absolute top-0 right-0 w-40 sm:w-64 h-40 sm:h-64 bg-gradient-to-br from-emerald-50 to-teal-50 rounded-bl-[100%] -mr-5 -mt-5 sm:-mr-10 sm:-mt-10 -z-0"></div>
                    ...
                    <div class="relative z-10 text-center space-y-6 sm:space-y-8">
                        ...
                        <div class="space-y-2">
                            <h2 class="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">Punya Kode Kelas?</h2>
                            <p class="text-xs sm:text-sm text-slate-500 font-medium px-4 sm:px-0">Masukkan kode yang diberikan dosen untuk bergabung.</p>
                        </div>

                        <form action="{{ route('mahasiswa.join.kelas') }}" method="POST" class="max-w-md mx-auto space-y-4 sm:space-y-6" id="join-form">
                            @csrf
                            <div id="field-code" class="group cursor-text p-4 rounded-2xl bg-slate-50 border-2 border-transparent transition-all duration-300 relative">
                                <label class="block text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest mb-2 sm:mb-3">Kode Akses</label>
                                <input type="text" id="class-code" name="code" placeholder="Contoh: X7Y99" class="w-full text-center text-2xl sm:text-3xl font-black uppercase tracking-[0.1em] sm:tracking-[0.2em] py-3 sm:py-5 border-b-4 border-slate-200 bg-transparent text-slate-800 placeholder-slate-300 focus:outline-none focus:border-emerald-500 transition-all font-mono" maxlength="8" autocomplete="off" />
                            </div>

                            <button type="button" id="btn-submit-kode" class="w-full bg-slate-900 text-white py-3 sm:py-4 rounded-xl sm:rounded-2xl font-bold text-xs sm:text-sm uppercase tracking-widest hover:bg-emerald-600 hover:shadow-lg hover:shadow-emerald-200 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-2 sm:gap-3 cursor-pointer pointer-events-auto">
        ...
        <script>
            ...
            function resetIdleTimer() {
                ...
                idleTimer = setTimeout(() => {
                    if (currentStep === "ASK_CODE") bicara("Waktu tunggu habis. Silakan sebutkan kode kelas Anda.");
                    else if (currentStep === "CONFIRM_CODE") bicara("Katakan Gabung, atau Ulang.");
                }, 180000); 
            }
            ...
            window.validasiAkhir = function() {
                ...
                if (code.length < 3) {
                    ...
                    bicara("Kode kurang lengkap. Sebutkan ulang.");
                    return;
                }
            ...
            window.navigasiKe = function(nomor) {
                ...
                if (nomor === 0) {
                    ...
                    setTimeout(() => { window.location.href = "{{ route('courses.index') }}"; }, 400); 
                } 
            }
            ...
            function parseKode(teks) {
                ...
                return hasil.replace(/[^a-z0-9]/gi, '').toUpperCase().substring(0, 8); 
            }
                ...
                rec.onresult = (event) => {
                    ...
                    if (currentStep === 'ASK_CODE') {
                        if (hasilTerakhir.includes("ulang")) {
                            synth.cancel(); resetMic(); inputCode.value = ""; bicara("Sebutkan ulang kode Anda."); return;
                        }

                        let codeFix = parseKode(hasilTerakhir);
                        if (codeFix.length > 0) {
                            inputCode.value = codeFix;
                            clearTimeout(typingTimer);
                            typingTimer = setTimeout(() => {
                                ...
                                bicara(`Kode Anda ${ejaan}. Katakan Gabung, atau Ulang?`);
                            }, 1500); 
                        }
                    } 
                        ...
                        if (pilihBenar && !pilihUlang) {
                            ...
                            inputCode.value = ""; currentStep = 'ASK_CODE';
                            fieldCode.classList.remove("border-emerald-500");
                            fieldCode.classList.add("border-blue-600");
                            bicara("Mari ulangi. Sebutkan kode kelas Anda.");
                        } else {
